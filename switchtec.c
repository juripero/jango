...
static struct switchtec_user *stuser_create(struct switchtec_dev *stdev)
{
	...
	if (!stuser)
    {   // KW_DB
        dev_info(&stdev->dev, "    -ENOMEM (Ln %d)\n", __LINE__);  // KW_DB
...
static void init_pff(struct switchtec_dev *stdev)
{
	...
	for (i = 0; i < SWITCHTEC_MAX_PFF_CSR; i++) {
        /// KW_DB->
        /// u64 addr;
        /// addr = (u64 ) &stdev->mmio_pff_csr[i].vendor_id;
        /// dev_info(&stdev->dev, "%s(): csr#%d:    addr=x%llx (Ln %d)\n", __func__, i, addr, __LINE__);
        /// KW_DB<-
		reg = ioread16(&stdev->mmio_pff_csr[i].vendor_id);
        dev_info(&stdev->dev, "%s():   %d:  reg: x%X (Ln %d)\n", __func__, i, reg, __LINE__);  // KW_DB
		if (reg != PCI_VENDOR_ID_MICROSEMI)
			break;
	}

	stdev->pff_csr_count = i;
    ///stdev->pff_csr_count = 3; // KW_DB
    dev_info(&stdev->dev, "%s(): pff_csr_count=%d (Ln %d)\n", __func__, stdev->pff_csr_count, __LINE__);  // KW_DB
			      ...
			      struct pci_dev *pdev)
{
    ...
    dev_info(&stdev->dev, " res_start =x%lx\n", res_start); // KW_DB
    dev_info(&stdev->dev, " res_len =x%lx\n", res_len); // KW_DB
    ...
    ///dev_info(&stdev->dev, "%s(): Here(Ln %d)\n", __func__, __LINE__);  // KW_DB
    ...
    dev_info(&stdev->dev, "%s(): partition=%d (Ln %d)\n", __func__, stdev->partition, __LINE__);  // KW_DB
    dev_info(&stdev->dev, "%s(): partition_count=%d (Ln %d)\n", __func__, stdev->partition_count, __LINE__);  // KW_DB
    ///dev_info(&stdev->dev, "%s(): pff_csr_count=%d (Ln %d)\n", __func__, stdev->pff_csr_count, __LINE__);  // KW_DB
    dev_info(&stdev->dev, "%s(): mmio_pff_csr=x%llX (Ln %d)\n", __func__, (u64) stdev->mmio_pff_csr, __LINE__);  // KW_DB
    dev_info(&stdev->dev, "%s(): Byte 00 - 03: x%08X (Ln %d)\n", __func__, ioread32(stdev->mmio_pff_csr), __LINE__);  // KW_DB
    
    
    

    // dev_info(&stdev->dev, "%s():     VenId0: x%X (Ln %d)\n", __func__, ioread16(&stdev->mmio_pff_csr->vendor_id), __LINE__);  // KW_DB
    // dev_info(&stdev->dev, "%s():     VenId1: x%X (Ln %d)\n", __func__, ioread16(&stdev->mmio_pff_csr->vendor_id + 0x1000), __LINE__);  // KW_DB
    
    // dev_info(&stdev->dev, "%s():     DevId0: x%X (Ln %d)\n", __func__, ioread16(&stdev->mmio_pff_csr->device_id), __LINE__);  // KW_DB
    // dev_info(&stdev->dev, "%s(): pci_class: x%X (Ln %d)\n", __func__, ioread32(&stdev->mmio_pff_csr->pci_class), __LINE__);  // KW_DB
    
    // dev_info(&stdev->dev, "%s():     BAR 0: x%X (Ln %d)\n", __func__, ioread32(&stdev->mmio_pff_csr->pci_bar[0]), __LINE__);  // KW_DB
    // dev_info(&stdev->dev, "%s():     BAR 1: x%X (Ln %d)\n", __func__, ioread32(&stdev->mmio_pff_csr->pci_bar[1]), __LINE__);  // KW_DB
    // dev_info(&stdev->dev, "%s():     BAR 2: x%X (Ln %d)\n", __func__, ioread32(&stdev->mmio_pff_csr->pci_bar[2]), __LINE__);  // KW_DB
    // dev_info(&stdev->dev, "%s():     BAR 3: x%X (Ln %d)\n", __func__, ioread32(&stdev->mmio_pff_csr->pci_bar[3]), __LINE__);  // KW_DB
    // dev_info(&stdev->dev, "%s():     BAR 4: x%X (Ln %d)\n", __func__, ioread32(&stdev->mmio_pff_csr->pci_bar[4]), __LINE__);  // KW_DB
    // dev_info(&stdev->dev, "%s():     BAR 5: x%X (Ln %d)\n", __func__, ioread32(&stdev->mmio_pff_csr->pci_bar[5]), __LINE__);  // KW_DB
    
    // dev_info(&stdev->dev, "%s():     BAR 0: x%llX (Ln %d)\n", __func__, ioread64(&stdev->mmio_pff_csr->pci_bar64[0]), __LINE__);  // KW_DB
    // dev_info(&stdev->dev, "%s():     BAR 2: x%llX (Ln %d)\n", __func__, ioread64(&stdev->mmio_pff_csr->pci_bar64[1]), __LINE__);  // KW_DB
    // dev_info(&stdev->dev, "%s():     BAR 4: x%llX (Ln %d)\n", __func__, ioread64(&stdev->mmio_pff_csr->pci_bar64[2]), __LINE__);  // KW_DB
    
    dev_info(&stdev->dev, "%s():        port_cnt: x%X (Ln %d)\n", __func__, ioread32(&stdev->mmio_part_cfg->port_cnt), __LINE__);  // KW_DB
    dev_info(&stdev->dev, "%s(): vep_pff_inst_id: x%X (Ln %d)\n", __func__, ioread32(&stdev->mmio_part_cfg->vep_pff_inst_id), __LINE__);  // KW_DB
    ...
    // KW_DB->
    dev_info(&stdev->dev, "%s(): Size of (struct pff_csr_regs) = x%lx (Ln %d)\n", __func__, sizeof(struct pff_csr_regs), __LINE__);
    for (i=0; i<stdev->pff_csr_count;i++)
    {
        dev_info(&stdev->dev, "----------------------------------\n");
        dev_info(&stdev->dev, "%s(): csr#%d:  VenId   : x%X (Ln %d)\n", __func__, i, ioread16(&stdev->mmio_pff_csr[i].vendor_id), __LINE__);
        dev_info(&stdev->dev, "%s(): csr#%d:  VenId   : x%X (Ln %d)\n", __func__, i, ioread16(&stdev->mmio_pff_csr[i].device_id), __LINE__);
        dev_info(&stdev->dev, "%s(): csr#%d:  PciClass: x%X (Ln %d)\n", __func__, i, ioread32(&stdev->mmio_pff_csr[i].pci_class), __LINE__);
        
        dev_info(&stdev->dev, "%s():     BAR 0: x%X (Ln %d)\n", __func__, ioread32(&stdev->mmio_pff_csr[i].pci_bar[0]), __LINE__);  // KW_DB
        dev_info(&stdev->dev, "%s():     BAR 1: x%X (Ln %d)\n", __func__, ioread32(&stdev->mmio_pff_csr[i].pci_bar[1]), __LINE__);  // KW_DB
        dev_info(&stdev->dev, "%s():     BAR 2: x%X (Ln %d)\n", __func__, ioread32(&stdev->mmio_pff_csr[i].pci_bar[2]), __LINE__);  // KW_DB
        dev_info(&stdev->dev, "%s():     BAR 3: x%X (Ln %d)\n", __func__, ioread32(&stdev->mmio_pff_csr[i].pci_bar[3]), __LINE__);  // KW_DB
        dev_info(&stdev->dev, "%s():     BAR 4: x%X (Ln %d)\n", __func__, ioread32(&stdev->mmio_pff_csr[i].pci_bar[4]), __LINE__);  // KW_DB
        dev_info(&stdev->dev, "%s():     BAR 5: x%X (Ln %d)\n", __func__, ioread32(&stdev->mmio_pff_csr[i].pci_bar[5]), __LINE__);  // KW_DB
        
        dev_info(&stdev->dev, "%s():     BAR 0: x%llX (Ln %d)\n", __func__, ioread64(&stdev->mmio_pff_csr[i].pci_bar64[0]), __LINE__);  // KW_DB
        dev_info(&stdev->dev, "%s():     BAR 2: x%llX (Ln %d)\n", __func__, ioread64(&stdev->mmio_pff_csr[i].pci_bar64[1]), __LINE__);  // KW_DB
        dev_info(&stdev->dev, "%s():     BAR 4: x%llX (Ln %d)\n", __func__, ioread64(&stdev->mmio_pff_csr[i].pci_bar64[2]), __LINE__);  // KW_DB
			       ...
			       const struct pci_device_id *id)
{
	...
	// if (pdev->class == (PCI_CLASS_BRIDGE_OTHER << 8))    // KW_DB
		// request_module_nowait("ntb_hw_switchtec");       // KW_DB
    ...
    dev_info(&stdev->dev, "==========================================\n");  // KW_DB
    dev_info(&stdev->dev, "%s(): class[x%x] (Ln %d)\n", __func__, pdev->class, __LINE__);  // KW_DB
    dev_info(&stdev->dev, "%s(): Calling switchtec_init_pci() (Ln %d)\n", __func__, __LINE__);  // KW_DB
	...
	pci_enable_pcie_error_reporting(pdev);   /// KW_DB_20240815 - 6.8.0-40-generic not supporting it
	pci_save_state(pdev);
...
static void switchtec_pci_disable(struct pci_dev *pdev)
{
	...
	if (pci_is_enabled(pdev)) {
		pci_disable_pcie_error_reporting(pdev);  /// KW_DB_20240815 - 6.8.0-40-generic not supporting it
...
static void switchtec_pci_error_resume(struct pci_dev *pdev)
{
	...
	pci_aer_clear_nonfatal_status(pdev);  /// KW_DB - Not in Kernel 5.4.104 
    ///pci_cleanup_aer_uncorrect_error_status(pdev);  // KW_DB - Kernel 5.4.104 has this.
}
